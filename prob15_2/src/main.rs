...
fn bfs_oxygen(nodes: &HashMap<(i32, i32), Node>) -> Result<i32, String> {
    let mut iters = 0;
    let mut checked_nodes = HashSet::new();
    for ((x, y), n) in nodes {
        if n.val == 2 {
            checked_nodes.insert((*x,*y));
        }
    }
...
fn main() -> Result<(), String> {
    ...
    print!("{}[2J", 27 as char);
    println!("Time until O2 fills up: {}", bfs_oxygen(&map_region(&mut m)?)?);
